...
// Auth middleware: accepts both JWT and API keys
documentsRouter.use("*", combinedAuthMiddleware);

// List documents and folders (supports ?recursive=true for a single-call tree load)
documentsRouter.get(
  "/list",
  requireAnyPermission(["documents:read", "folders:read"]),
  async (c) => {
...
documentsRouter.post(
  "/rename",
  requireAnyPermission(["documents:update", "folders:update"]),
  async (c) => {
    ...
    if (newName !== undefined) {
      ...
      newPath =
        parentDir === "/" ? `/${finalName}` : `${parentDir}/${finalName}`;
    } else if (rawNewPath) {
      newPath = rawNewPath;
    } else {
...
documentsRouter.get(
  ...
  async (c) => {
    ...
    try {
      const deleted = documentQueries.listDeletedDocuments.all(
        user.currentOrgId,
      );
...
documentsRouter.post(
  ...
  requirePermission("documents:delete"),
  async (c) => {
    const parsed = permanentlyDeleteDocumentSchema.safeParse(
      await c.req.json(),
    );
    if (!parsed.success) {
      return c.json({ error: z.treeifyError(parsed.error) }, 400);
...
documentsRouter.post(
  "/duplicate",
  requireAnyPermission(["documents:create", "folders:create"]),
  async (c) => {
